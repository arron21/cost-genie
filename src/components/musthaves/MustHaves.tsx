...
const MustHaves = () => {
  ...
  useEffect(() => {
    const fetchMustHaves = async () => {
      ...
      try {
        ...
        const mustHavesList: CostRecord[] = querySnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data(),
          date: doc.data().createdAt.toDate()
        })) as CostRecord[];
        
        setMustHaves(mustHavesList);
      } catch (error) {
        console.error('Error fetching must-haves:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Calculate total yearly cost of all must-haves
  useEffect(() => {
    if (!userProfile || mustHaves.length === 0) return;
    
    const calculateTotalYearlyCost = () => {
      let total = 0;
      
      mustHaves.forEach(cost => {
  ...
  }, [mustHaves, userProfile, afterTaxIncome]);

  const toggleMustHave = async (costId: string, currentMustHave: boolean = true) => {
    if (!currentUser) return;
    
    try {
      // Use the imported toggleCostMustHave function instead
      await toggleCostMustHave(costId, !currentMustHave);
      
      // If unmarked as must-have, remove from must-haves list
      if (currentMustHave) {
        setMustHaves(prev => prev.filter(cost => cost.id !== costId));
      }
    } catch (error) {
      console.error('Error toggling must-have status:', error);
    }
  };
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      {mustHaves.length === 0 ? (
        <div className="text-center mt-8">
          <p className="text-gray-500 mb-4">No must-have items yet</p>
          <Link 
            to="/history" 
            className="text-indigo-600 hover:text-indigo-800 underline"
          >
            Go to history to mark items as must-have
          </Link>
        </div>
      ) : (
